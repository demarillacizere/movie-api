...
class MiddlewareAfter
{
    ...
    public function __invoke(Request $request, RequestHandler $handler): ResponseInterface
    {
        ...
        // Log the response only if it hasn't been logged yet
        if (!$request->getAttribute('responseLogged')) {
            $this->logResponse($response);
            $request = $request->withAttribute('responseLogged', true); // Mark as logged
        }
